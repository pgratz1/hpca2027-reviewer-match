...
identities win, as in dblp_overrides.csv) unless --overwrite is given.
    ...
    python -m scripts.resolve_trc_members
    python -m scripts.resolve_trc_members --out /tmp/trc-dry-run.csv   # leave the roster alone
    python -m scripts.resolve_trc_members --no-network                 # cached lookups only
    python -m scripts.resolve_trc_members --limit 5                    # try a few rows first
"""
...
DEFAULT_TRC = input_path("hpca2027-trc - hpca2027-trc.csv")
...
DEFAULT_SENIORITY = report_path("reviewer_seniority.csv")
