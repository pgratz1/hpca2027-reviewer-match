"""Classify accepted PC members by publication seniority from DBLP history.
    ...
    python -m scripts.classify_reviewers
...
from reviewer_match.reviewers import DEFAULT_OVERRIDES, Reviewer, load_reviewers
...
DEFAULT_VENUE_CACHE = cache_path("dblp_venue_cache.json")
DEFAULT_SNAPSHOT_CACHE = cache_path("dblp_snapshot_cache.json")
DEFAULT_OUT = report_path("reviewer_seniority.csv")
DEFAULT_RESERVE_OUT = report_path("reserve_seniority.csv")
DEFAULT_RESERVE_INFO = report_path("reserve_reviewer_info.csv")
DEFAULT_DATA = input_path("hpca2027-data.json")
DEFAULT_PCDB = input_path("PCDB_with_emails.csv")
