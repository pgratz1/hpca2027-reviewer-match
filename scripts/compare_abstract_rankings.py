"""Build a blinded chair-rating sample for title-only vs abstract rankings.
    ...
    python -m scripts.compare_abstract_rankings \
...
from reviewer_match.paths import cache_path, evaluation_path, input_path
...
from reviewer_match.paper_matching import PAPER_POLICIES, eligible_scores, load_papers
...
def main() -> int:
    ...
    parser.add_argument(
        "--rating-out", default=evaluation_path("abstract-evaluation-ratings.csv")
    )
    parser.add_argument(
        "--comparison-out", default=evaluation_path("abstract-evaluation-key.csv")
    )
